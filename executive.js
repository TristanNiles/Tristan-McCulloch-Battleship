        ...
        if (AI == 1){
            if (g_currentPlayer == 1) {
                ...
                if (g_currShipLength > g_maxShips) {
                    ...
                    for (let i = g_currShipLength; i<=g_maxShips; i= g_currShipLength) {
 ...
 */
 function fire(arr, pos) {
    ...
    if (arr[pos] == 1 || arr[pos] == 2 || arr[pos] == 3 || arr[pos] == 4 || arr[pos] == 5 || arr[pos] == 6) { //only executes if un-hit ship is detected
        temp_ship = arr[pos];        
		arr[pos] = 7;
        if (arr == g_player2arr || g_opponent == "human") { 
            g_hit = 1;
        }
        ...
        arr[pos] = 8;
        if (g_opponent == "human" || arr == g_player2arr) {
            g_hit = 0;
        }
        return true;
    }
...
function scoreCheck(arr, ship) {
  ...
  if(arr == g_player2arr)  {
    ...
    if(count == 0)  {
        ...
        if (arr == g_player1arr && g_opponent == "human") {
	       g_hit = 2;
        }
    }  
  }
